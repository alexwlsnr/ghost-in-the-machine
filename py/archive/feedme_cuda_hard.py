#!/usr/bin/env python3
"""
Autoregressive character-level model for Z80 — CUDA-enabled variant.

Same as feedme.py but with GPU (CUDA) support. Use --device cpu to
force CPU mode, otherwise defaults to CUDA when available.

Differences from feedme.py:
- Detects and uses CUDA by default
- Moves model and tensors to GPU for training/inference
- Handles checkpoint device mapping correctly
"""
...
class AutoregressiveModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, use_int: bool = False, quant_temp: float = 1.0,
                weight_bits: int = 2) -> torch.Tensor:
        if use_int:
            return self._forward_int(x, weight_bits=weight_bits)
    ...
    def _forward_int(self, x: torch.Tensor, weight_bits: int = 2) -> torch.Tensor:
        ...
        for i, layer in enumerate(self.layers):
            ...
            w_quant = torch.clamp(torch.round(w / scale), min_val, max_val)
    ...
    def get_quantized_params(self, weight_bits: int = 2) -> dict:
        """Extract quantized weights (2-bit or 4-bit)."""
        params = {}
        max_val = 2 ** (weight_bits - 1) - 1
        min_val = -(2 ** (weight_bits - 1))
...
def generate_response(model: AutoregressiveModel, query: str,
    ...
    with torch.no_grad():
        for _ in range(max_len):
            ...
            full_input = np.concatenate([query_vec, context_vec])
            x = torch.tensor(full_input, dtype=torch.float32, device=device).unsqueeze(0)
...
def train_chunked(chunk_size: int = 1000, epochs_per_chunk: int = 100, lr: float = 0.005,
                  save_best: bool = False, device: str = 'cuda',
                  weight_bits: int = 2, hidden_sizes: list = None):
    ...
    print(f"Weight bits: {weight_bits} (values: {list(range(-2**(weight_bits-1), 2**(weight_bits-1)))} )")
    ...
    for chunk_num in range(total_chunks):
        ...
        for epoch in range(epochs_per_chunk):
            try:
                ...
                quant_temp = 0.7 + 0.3 * min(1.0, epoch / (epochs_per_chunk * 0.8))

                outputs = model(X, quant_temp=quant_temp)
                ce_loss = criterion(outputs, y)
                quant_loss = model.compute_quantization_loss() * 0.50
                ...
                if (epoch + 1) % 10 == 0:
                    with torch.no_grad():
                        ...
                        int_outputs = model(X, use_int=True, weight_bits=weight_bits)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Train autoregressive model (CUDA-enabled)')
    ...
    parser.add_argument('--chunk', '-c', type=int, default=0,
                        help='Chunk size for streaming (0 = load all as one chunk)')
    ...
    parser.add_argument('--chat', action='store_true',
                        help='Interactive chat after training')
    parser.add_argument('--bits', '-b', type=int, default=2, choices=[2, 4],
                        help='Weight quantization bits (2 or 4)')
    parser.add_argument('--hidden', type=str, default='256,192,128',
    ...
    model = train_chunked(chunk_size=args.chunk, epochs_per_chunk=args.epochs,
