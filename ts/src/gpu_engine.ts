...
interface Pipelines {
  ...
  matF32: GPUComputePipeline; matI4g: GPUComputePipeline; matI8: GPUComputePipeline;
...
interface WeightTensor {
  ...
  int8Buf?: GPUBuffer;   // int8 weights (bound as array<u32>)
  paramBuf?: GPUBuffer;  // int8 per-tensor param buffer {in,out,scale,has_bias}
  biasBuf: GPUBuffer;    // fp32 bias (may be all zeros)
  dtype: string;
  inDim: number; outDim: number;
}

// A pre-built GPU operation: pipeline + bind group + workgroup count.
// Created once at load time, dispatched every token — eliminates per-token
// createBindGroup() overhead which was the main CPU bottleneck (~96 × ~1ms).
interface GPUOp { pip: GPUComputePipeline; bg: GPUBindGroup; wgs: number; }

// Per-layer pre-built ops (in execution order)
interface LayerOps {
  ln1: GPUOp; q: GPUOp; k: GPUOp;
  wkvK: GPUOp; v: GPUOp; wkvV: GPUOp;
  attn: GPUOp; o: GPUOp; addResid: GPUOp;
  ln2: GPUOp; ff1: GPUOp; relu: GPUOp; ff2: GPUOp;
}
...
export class GPUEngine {
  ...
  // Pre-built ops — created once at init, dispatched every token
  private embedOp!: GPUOp;
  private layerOps: LayerOps[] = [];
  private finalLnOp!: GPUOp;
  private headOp!: GPUOp;
  ...
  private _buildOps(): void {
    ...
    const mkBG = (pipeline: GPUComputePipeline, bufs: GPUBuffer[]): GPUBindGroup =>
      device.createBindGroup({
        layout: pipeline.getBindGroupLayout(0),
        entries: bufs.map((b, i) => ({ binding: i, resource: { buffer: b } })),
      });

    const mkMat = (wt: WeightTensor, input: GPUBuffer, output: GPUBuffer): GPUOp => {
  ...
  destroy(): void {
    ...
    for (const t of [...this.qT, ...this.kT, ...this.vT, ...this.oT, ...this.ff1T, ...this.ff2T]) {
      t.f32Buf?.destroy(); t.nibBuf?.destroy(); t.sclBuf?.destroy();
      t.int8Buf?.destroy(); t.paramBuf?.destroy(); t.biasBuf.destroy();
    }
    this.device.destroy();
  }
  ...
  private _run(enc: GPUCommandEncoder, op: GPUOp): void {
  ...
  private async _step(token: number): Promise<Float32Array> {
    ...
    for (let li = 0; li < nl; li++) {
      ...
      this._run(enc, L.ln1);
      this._run(enc, L.q);
      this._run(enc, L.k);     this._run(enc, L.wkvK);
      this._run(enc, L.v);     this._run(enc, L.wkvV);
      this._run(enc, L.attn);
      this._run(enc, L.o);     this._run(enc, L.addResid);
      this._run(enc, L.ln2);
      this._run(enc, L.ff1);   this._run(enc, L.relu);
      this._run(enc, L.ff2);   this._run(enc, L.addResid);
    }
