...
export async function* generate(
  ...
  if (cache !== undefined) {
    ...
    const sample = (lg: Float32Array): number => {
      let maxV = -Infinity;
      for (let i = 0; i < arch.vocab_size; i++) if (lg[i] > maxV) maxV = lg[i];
      let sum = 0;
      const probs = new Float64Array(arch.vocab_size);
      for (let i = 0; i < arch.vocab_size; i++) { probs[i] = Math.exp((lg[i] - maxV) / temp); sum += probs[i]; }
